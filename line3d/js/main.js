...
function onTouchMove(e) {
	...
	mouseY = ( e.touches[0].clientY - windowHalfY ) * 10;
			...
			function render() {
				...
				scene.remove(lines);
				lines.geometry.vertices.push(dummy.position.clone());

			    lines.geometry.vertices = lines.geometry.vertices;

			    var gem = new THREE.Geometry();
			    gem.vertices = lines.geometry.vertices;
			    if(gem.vertices.length > 1000)gem.vertices.shift();
				lines = new THREE.Line( gem, lines.material);
				...
				// if(iOS)
				// 	camera.translateY( -88000 * (1/windowHalfY));
				// else
				// 	camera.translateY( -85000 * (1/windowHalfY));
				// camera.translateX( -38000 * (1/windowHalfY));
